...
//修改信息类, 修改教务人员的登录密码
public class PasswordChangePage extends JPanel implements ActionListener{
	...
	//变量定义
		...
		TeachingManager teach;//管理员对象
		//构造方法
		public PasswordChangePage(TeachingManager teach) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
					//判断原密码是否正确
					if(pwdOld.getText().equals(teach.getPassWord())) {
						//判断新密码和确认密码是否一致
						if (pwdNew.getText().equals(pwdCheck.getText())) {
							//操作对象是教务管理员
							DBHelper.ChangeInfo(pwdChange, "teachingmanager", "employeeid", teach.getEmployeeID());
							//修改成功提示
